...
def layer_aicl_forward_default(self, hidden_states, mask, ref_hidden_states, *args, **kw_args):
    '''
        hidden_states: [batch, seq_len, hidden_size]
        mask: [(1, 1), seq_len, seq_len]
    '''
    ...
    #         else:
    #             cross_input = self.post_cross_attention_layernorm(hidden_states)
    #             kw_args_new = {"layer_id": kw_args['layer_id']}
    #             attention_output = self.cross_attention(cross_input, mask, ref_hidden_states, **kw_args_new)
    #             hidden_states = hidden_states + self.cross_attention_proj_out(attention_output)
    if self.is_decoder:
        encoder_outputs = kw_args['encoder_outputs']
        if encoder_outputs is not None:
            assert 'cross_attention_mask' in kw_args
...
HOOKS_DEFAULT = {
    ...
    'layer_forward': layer_aicl_forward_default,
    # 'layer_aicl_forward': layer_aicl_forward_default,
}
