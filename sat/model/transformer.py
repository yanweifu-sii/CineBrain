...
class BaseTransformerLayerAICL(torch.nn.Module):
    def __init__(
            self,
            hidden_size,
            num_attention_heads,
        ...
        super(BaseTransformerLayerAICL, self).__init__()
        # Set output layer initialization if not provided.
        if output_layer_init_method is None:
            output_layer_init_method = init_method
        self.layer_id = layer_id
        ...
        if is_AICL:
            ...
            # self.cross_attention_adaLN_modulation = torch.nn.Sequential(
            #     torch.nn.SiLU(), 
            #     torch.nn.Linear(512, 2 * hidden_size)
            # )
            self.post_cross_attention_layernorm = layernorm(hidden_size, eps=layernorm_epsilon)
            self.cross_attention_proj_out = zero_module(torch.nn.Linear(hidden_size, hidden_size))
        # Cross attention.
        if self.is_decoder:
            self.cross_attention = CrossAttention(
                hidden_size,
    ...
    def forward(self, hidden_states, mask, *args, **kw_args):
        return HOOKS_DEFAULT['layer_aicl_forward'](self, hidden_states, mask, *args, **kw_args)


class BaseTransformerAICL(torch.nn.Module):
    def __init__(self,
                 num_layers,
                 vocab_size,
                 hidden_size,
        ...
        super(BaseTransformerAICL, self).__init__()
        ...
        def get_layer(layer_id):
            return BaseTransformerLayerAICL(
                hidden_size,
                num_attention_heads,
                attention_dropout_prob,
                output_dropout_prob,
                ...
                skip_init=skip_init,
                device=device,
                is_AICL=is_AICL if layer_id%2==0 else False
            )
            ...
            def custom(start, end, kw_args_index, cross_layer_index):
                def custom_forward(*inputs):
                    ...
                    for i, layer in enumerate(layers_):
                        ...
                        if 'layer_forward' in self.hooks:
                            layer_ret = self.hooks['layer_forward'](
                                x_, mask, inputs[-1], layer_id=layer.layer_id,
                                **kw_args, position_ids=position_ids, **output_cross_layer,
                                output_this_layer=output_this_layer_obj,
                                output_cross_layer=output_cross_layer_obj
                            )
...
class BaseTransformerLayerControl(torch.nn.Module):
    def __init__(
            self,
            hidden_size,
            num_attention_heads,
        ...
        super(BaseTransformerLayerControl, self).__init__()
        # Set output layer initialization if not provided.
        if output_layer_init_method is None:
            output_layer_init_method = init_method
        self.layer_id = layer_id
        ...
        if self.is_control:
            # Layernorm on the input data.
            self.control_input_layernorm = layernorm(hidden_size, eps=layernorm_epsilon)

            # self.control_in_linear = zero_module(torch.nn.Linear(hidden_size, hidden_size))

            # Self attention.
            self.control_attention = SelfAttention(
                hidden_size,
                num_attention_heads,
                attention_dropout_prob,
                output_dropout_prob,
            ...
            # Layernorm on the input data.
            self.control_post_attention_layernorm = layernorm(hidden_size, eps=layernorm_epsilon)
            if self.layernorm_order == 'sandwich':
                self.control_third_layernorm = layernorm(hidden_size, eps=layernorm_epsilon)
                self.control_fourth_layernorm = layernorm(hidden_size, eps=layernorm_epsilon)

            # MLP
            self.control_mlp = MLP(
                hidden_size,
                output_dropout_prob,
                init_method,
                inner_hidden_size=inner_hidden_size,
            ...
            self.control_skip_norm = torch.nn.LayerNorm(
                hidden_size, elementwise_affine=False, eps=1e-6, device=device, dtype=params_dtype
            )
            self.control_out_linear = zero_module(torch.nn.Linear(hidden_size, hidden_size))
            self.control_adaLN_modulation = torch.nn.Sequential(
                torch.nn.SiLU(), 
                torch.nn.Linear(512, 12 * hidden_size)
            )
...
class BaseTransformerControlNet(torch.nn.Module):
    def __init__(self,
                 num_layers,
                 vocab_size,
                 hidden_size,
        ...
        super(BaseTransformerControlNet, self).__init__()
        ...
        def get_layer(layer_id):
            return BaseTransformerLayerControl(
                hidden_size,
                num_attention_heads,
                attention_dropout_prob,
                output_dropout_prob,
                ...
                skip_init=skip_init,
                device=device,
                is_control=True if layer_id < 18 else False
            )
    ...
    def forward(self, input_ids, position_ids, attention_mask, *,
        ...
        if position_embeddings is not None:
            hidden_states = hidden_states + position_embeddings
        hidden_states = self.embedding_dropout(hidden_states)
        control_hidden_states = kw_args["control"]
        ...
        if self.checkpoint_activations:
            ...
            def custom(start, end, kw_args_index, cross_layer_index):
                def custom_forward(*inputs):
                    layers_ = self.layers[start:end]
                    x_, mask, control_hidden_states = inputs[0], inputs[1], inputs[2]
                    ...
                    for i, layer in enumerate(layers_):
                        ...
                        if 'layer_forward' in self.hooks:
                            layer_ret = self.hooks['layer_forward'](
                                x_, mask, control_hidden_states, layer_id=layer.layer_id,
                                **kw_args, position_ids=position_ids, **output_cross_layer,
                                output_this_layer=output_this_layer_obj,
                                output_cross_layer=output_cross_layer_obj
                            )
                        ...
                        if isinstance(layer_ret, tuple):
                            layer_ret, control = layer_ret[0], layer_ret[1] # for legacy API
                        x_, output_this_layer, output_cross_layer = layer_ret, output_this_layer_obj, output_cross_layer_obj
                        if output_hidden_states:
                            output_this_layer['hidden_states'] = x_
                        output_per_layers_part.append(output_this_layer)
                    ...
                    for k in output_cross_layer:
                        ...
                        output_cross_layer[k] = len(flat_outputs) - 1
                    # --------------------
                    return (x_, control, output_per_layers_part, output_cross_layer, *flat_outputs)
                return custom_forward
            ...
            output_this_layer = []
            while l < num_layers:
                args = [hidden_states, attention_mask, control_hidden_states]
                # flatten kw_args and output_cross_layer
                flat_inputs, kw_args_index, cross_layer_index = [], {}, {}
                for k, v in kw_args.items():
                    flat_inputs.append(v)
                ...
                if l + self.checkpoint_skip_layers >= num_layers:
                    # no checkpointing
                    hidden_states, control_hidden_states, output_per_layers_part, output_cross_layer, *flat_outputs = \
                    custom(l, l + chunk_length, kw_args_index, cross_layer_index)(*args, *flat_inputs)
                else:
                    hidden_states, control_hidden_states, output_per_layers_part, output_cross_layer, *flat_outputs = \
                    checkpoint(custom(l, l + chunk_length, kw_args_index, cross_layer_index), *args, *flat_inputs)
            ...
            for i, layer in enumerate(self.layers):
                ...
                if 'layer_forward' in self.hooks: # customized layer_forward
                    ...
                    layer_ret = layer(*args, layer_id=torch.tensor(i), **kw_args, position_ids=position_ids, **output_cross_layer,
                        output_this_layer=output_this_layer_obj, output_cross_layer=output_cross_layer_obj)
                if isinstance(layer_ret, tuple):
                    layer_ret, control_hidden_states = layer_ret[0],layer_ret[1] # for legacy API
                hidden_states, output_this_layer, output_cross_layer = layer_ret, output_this_layer_obj, output_cross_layer_obj
