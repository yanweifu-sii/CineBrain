...
class BrainTransformer(nn.Module):
    def __init__(self,
                 ...
                 clip_dim: int = 1152,
                 nhead: int = 32,
                 separate_layers: int = 8,
                 combined_layers: int = 8,
                 dropout: float = 0.01):
        super().__init__()
        
        # Tokenizers
        self.fmri_tokenizer = fMRITokenizer(in_channels=in_fmri_channels, 
                                          seq_len=fmri_seq_len,
                                          embed_dim=embed_dim, 
                                          num_spatial=num_spatial)
        self.eeg_tokenizer = EEGTokenizer(d_model=embed_dim, 
                                        num_spatial=num_spatial)

        # Separate processing layers (first 8 layers)
        self.eeg_layers = nn.ModuleList([
            TransformerLayer(embed_dim, nhead, embed_dim * 4, dropout)
            for _ in range(separate_layers)
        ])
        ...
        # Combined processing layers (last 8 layers)
        self.combined_layers = nn.ModuleList([
            TransformerLayer(embed_dim, nhead, embed_dim * 4, dropout)
            for _ in range(combined_layers)
        ])

        # Position embeddings
        self.eeg_pos_emb = nn.Parameter(torch.randn(1, num_spatial + 1, embed_dim))
        self.fmri_pos_emb = nn.Parameter(torch.randn(1, num_spatial + 1, embed_dim))
        self.combined_pos_emb = nn.Parameter(torch.randn(1, 2 * (num_spatial + 1), embed_dim))
    ...
    def forward(self, eeg_x: torch.Tensor, fmri_x: torch.Tensor,
        ...
        # Tokenize inputs
        eeg_tokens = self.eeg_tokenizer(eeg_x)
        fmri_tokens = self.fmri_tokenizer(fmri_x)
        ...
        eeg = torch.cat([cls_eeg, eeg_tokens], dim=1)
        fmri = torch.cat([cls_fmri, fmri_tokens], dim=1)

        # Add position embeddings
        eeg = eeg + self.eeg_pos_emb
        fmri = fmri + self.fmri_pos_emb

        # Separate processing (first 8 layers)
        for eeg_layer, fmri_layer in zip(self.eeg_layers, self.fmri_layers):
            eeg = eeg_layer(eeg, self_mask=mask)
            fmri = fmri_layer(fmri, self_mask=mask)

        # Combine and process together (last 8 layers)
        combined = torch.cat([eeg, fmri], dim=1)
        combined = combined + self.combined_pos_emb
        
        for layer in self.combined_layers:
            combined = layer(combined, self_mask=mask)

        # Split back
        eeg_out, fmri_out = combined[:, :self.num_spatial+1], combined[:, self.num_spatial+1:]

        # Extract CLS tokens and features
        eeg_cls = eeg_out[:, 0]
        fmri_cls = fmri_out[:, 0]
...
class BrainTransformer_concat_dim(nn.Module):
    def __init__(self,
                 in_fmri_channels: int = 5,
                 fmri_seq_len: int = 8405,
                 num_spatial: int = 226,
        ...
        # Combined processing layers (with doubled embed_dim)
        self.combined_layers = nn.ModuleList([
            TransformerLayer(embed_dim * 2, nhead, embed_dim * 8, dropout)
            for _ in range(combined_layers)
        ])
        ...
        self.eeg_pos_emb = nn.Parameter(torch.randn(1, num_spatial + 1, embed_dim))
        self.fmri_pos_emb = nn.Parameter(torch.randn(1, num_spatial + 1, embed_dim))
        self.combined_pos_emb = nn.Parameter(torch.randn(1, num_spatial + 1, embed_dim * 2))
    ...
    def forward(self, eeg_x: torch.Tensor, fmri_x: torch.Tensor,
        ...
        combined = torch.cat([eeg, fmri], dim=-1)  # concat on feature dimension
        combined = combined + self.combined_pos_emb
        
        for layer in self.combined_layers:
            combined = layer(combined, self_mask=mask)

        # Split back on feature dimension
        eeg_out = combined[..., :self.embed_dim]
        fmri_out = combined[..., self.embed_dim:]
